...
const EditarPlantioCanteiro = ({ itemParaEditar, onSave, onCancel }) => {
  ...
  return (
    <FormGeral
      ...
      actions={[
        { type: 'button', variant: 'action-secondary', children: 'Cancelar', onClick: onCancel },
        { type: 'submit', variant: 'primary', children: 'Salvar Alterações', disabled: !loteSelecionado }
      ]}
    ...
    >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={tableHeaderStyle}>
              <tr>
                ...
                <th style={{ ...tableCellStyle, textAlign: 'left' }}>Espécie</th>
            ...
            <tbody>
              {lotes.map((l) => (
                <tr key={l.id} style={{ backgroundColor: loteSelecionado?.id === l.id ? '#e8f5e9' : 'transparent' }}>
                  <td style={tableCellStyle}>{l.lote}</td>
                  <td style={tableCellStyle}>{l.nomeEspecie}</td>
                  <td style={{ ...tableCellStyle, textAlign: 'center' }}>{l.quantidade}</td>
                  ...
                  <td style={{ ...tableCellStyle, textAlign: 'center' }}>
                    <button 
                      ...
                      style={{
                          ...editButtonStyle,
                          backgroundColor: loteSelecionado?.id === l.id ? '#4CAF50' : '#e0e0e0',
      ...
      {loteSelecionado && (
        <>
          <div className="form-geral__campo--span-2" style={{ backgroundColor: '#e8f5e9', padding: '15px', borderRadius: '8px', borderLeft: '5px solid #4CAF50', marginTop: '10px' }}>
             <p style={{ margin: 0, color: '#2e7d32' }}>Editando Lote: <strong>{loteSelecionado.lote}</strong></p>
          </div>
          ...
          <div className="form-geral__campo--span-2">
            <Input
              ...
              onChange={(e) => setFormData(p => ({...p, dataPlantio: e.target.value}))}
              required
            />
          </div>
        </>
