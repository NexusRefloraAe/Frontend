...
  const [modalEdicaoAberto, setModalEdicaoAberto] = useState(false); // ✅ controle do modal de edição
...
  // ✅ Ao clicar em ✏️ Editar no modal de detalhes
  const handleEditarCanteiro = (canteiro) => {
    console.log("Abrir edição para:", canteiro);
    setCanteiroSelecionado(canteiro);
    setModalDetalheAberto(false); // fecha detalhes
    setModalEdicaoAberto(true);   // abre edição
  };
...
  // ✅ Após salvar no EditarCanteiro
  const handleSalvarEdicao = (canteiroAtualizado) => {
    // Atualiza a lista com os novos dados (vamos assumir que EditarCanteiro retorna o objeto atualizado)
    setCanteiros(prev =>
...
          item={canteiroSelecionado}
          ...
          // Estes são os campos que batem com a sua imagem
          camposDetalhes={[
            { label: 'Nome:', chave: 'NomeCanteiro' },
            // 'valorPadrao' é usado para textos fixos
...
          onEditar={handleEditarCanteiro}   // ✅ chama o handler local
          onExcluir={handleExcluirCanteiro}
          onExportar={handleExportarCanteiro}
          textoExclusao="o canteiro"
          mostrarAcoes={true}
          mostrarHistorico={true}
          mostrarExportar={true} // O ModalDetalheGenerico já coloca isso na direita
        />
      )}

      {/* ✅ MODAL DE EDIÇÃO — renderiza o EditarCanteiro como modal */}
...
              style={{ position: 'absolute', top: '12px', right: '12px', background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', zIndex: 10 }} // zIndex para ficar sobre o form
            >
              ✕
            </button>

            {/* ✅ Renderiza o seu componente EditarCanteiro */}
            <EditarCanteiro
              itemParaEditar={canteiroSelecionado}
a              onSave={handleSalvarEdicao}
