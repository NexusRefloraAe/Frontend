...
function ListaSementes({
  ...
  const dadosFormatados = (sementes || []).map((semente, index) => {
    const qtdTexto = semente.quantidadeAtualFormatada
      ...
      : semente.quantidade
      ? `${semente.quantidade} ${semente.unidadeDeMedida || ""}`
      : "0";

    return {
      ...semente,
      // ✅ CORREÇÃO: Cria um ID único combinando o UUID com o índice da linha
      id: semente.id ? `${semente.id}-${index}` : `row-${index}`,

      // Mantenha o ID original em uma propriedade separada se precisar dele para editar/excluir
      idEntidadeOriginal: semente.id,
      ...
      acoesRenderizadas: (
        <div style={{ display: "flex", gap: "15px", justifyContent: "center" }}>
          ...
          <button
            onClick={() => onSolicitarExclusao(semente)}
            style={{
              border: "none",
              background: "transparent",
              cursor: "pointer",
          ...
          {semente.idUltimoMovimentacao && (
            <button
              ...
              title={`Mudar de ${semente.finalidadeAtual} para ${
                semente.finalidadeAtual === "Plantio" ? "Teste" : "Plantio"
              }`}
            >
